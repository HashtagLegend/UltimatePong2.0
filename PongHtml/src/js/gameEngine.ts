...
export class GameEngine
{
    ...
    public gameLoop(): void{
        ...
        if((GameEngine.aiScore >= 3 || GameEngine.playerScore >= 3)&& GameEngine.gamecon == true){
            ...
            if(GameEngine.aiScore >= 3 && Ai.aiLvl != "mult")
            {                
                console.log("Ai win");
                axios.put("https://ponghighscorewebservice.azurewebsites.net/api/bruger/" + (document.getElementById("googleuserid").innerHTML).split(":")[1], {gamemode: "AI", Loss : 1})
            ...
            if(GameEngine.playerScore >= 3 && Ai.aiLvl != "mult")
            {               
                console.log("player win");
                axios.put("https://ponghighscorewebservice.azurewebsites.net/api/bruger/" + (document.getElementById("googleuserid").innerHTML).split(":")[1], {gamemode: "AI", Win : 1})
                .then(function (response) {
                console.log(response);
                })
                .catch(function (error) {
            ...
            if(GameEngine.aiScore >= 3 && Ai.aiLvl == "mult")
            {
                console.log("Multiplayer player1 win");
                  axios.put("https://ponghighscorewebservice.azurewebsites.net/api/bruger/" + (document.getElementById("googleuserid").innerHTML).split(":")[1], {gamemode: "Multiplayer", Loss : 1})
                    .then(function (response) {
                    console.log(response);
                    })
                    .catch(function (error) {
            ...
            if(GameEngine.playerScore >= 3 && Ai.aiLvl == "mult")
            {                
                    console.log("Multiplayer player2 win");
                    axios.put("https://ponghighscorewebservice.azurewebsites.net/api/bruger/" + (document.getElementById("googleuserid").innerHTML).split(":")[1], {gamemode: "Multiplayer", Win : 1})
                    .then(function (response) {
                    console.log(response);
                    })
                    .catch(function (error) {
